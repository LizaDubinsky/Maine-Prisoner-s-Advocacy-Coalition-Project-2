...
m <- prison_expenditures %>%
  filter(state == c("Massachusetts", "Connecticut")) %>%
...
new_england_jail_capacity <- incarc_trends_2 %>%
  filter(state == c("MA", "NH", "ME")) %>%
  group_by(state, year) %>%
  summarise(state_jail_capacity = (sum(jail_rated_capacity))/(sum(total_pop))*1000)
...
```{r}
prison_counts <- incarceration_trends %>%
  select(year, state, county_name, total_prison_pop, total_jail_pop, total_pop) %>%
  group_by(state, year) %>%
  summarise(total_incarcerated = sum(total_prison_pop) + sum(total_jail_pop))

parole_counts <- all_parole_years 
  
```


```{r}
maine_prison_counts <- maine_data %>%
  select(year, state, county_name, total_prison_pop, total_jail_pop, total_pop) %>%
  group_by(state, year) %>%
  summarise(total_incarcerated = sum(total_prison_pop + total_jail_pop))


maine_prison_counts %>% 
    group_by(year) %>% 
    summarise(total_incarcerated = sum(total_prison_pop))
```
