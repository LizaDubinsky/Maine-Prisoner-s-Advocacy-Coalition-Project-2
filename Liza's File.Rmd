...
new_england_crime_counts <- read.csv("incarceration_trends3.csv") %>%
  ...
  group_by(state, year) %>%
  summarise(state_index_crime = (sum(index_crime))/(sum(total_pop)))
...
p <- ggplot(
  ...
  geom_line() +
  labs(x = "Year", y = "Index Crimes Committed Per Capita", title = "Index Crimes Committed Per Capita in New England") +
  theme(legend.position = "top")
p + 
  geom_point() +
  transition_reveal(year)
...
p <- ggplot(
  ...
  geom_line() +
  labs(x = "Year", y = "Burglary Crime Count", title = "Burglary Crimes Committed in New England From 1970 to 2018") +
  theme(legend.position = "top")
p + 
  geom_point() +
  transition_reveal(year)
...
me_total_incarcerated <- incarceration_trends %>%
  ...
  group_by(year) %>%
  summarise(total_incarcerated = (round(
    (sum(total_prison_pop, na.rm = TRUE)) + sum(total_jail_pop, na.rm = TRUE)))/(sum(total_pop))
    )
...
p <- ggplot(
  ...
  geom_line() +
  labs(x = "Year", y = "Number of Incarcerated People Per Capita", title = "Number of Incaracerated People Per Capita in Maine From 1970 to 2018") +
  theme(legend.position = "top") +
...
jail_percentage <- incarc_trends_2 %>%
  ...
  filter(state == "ME") %>%
  group_by(year) %>%
  summarise(percent_filled = ((sum(total_jail_pop))/((sum(jail_rated_capacity))))*100)
...
p <- ggplot(
  ...
  geom_line() +
  labs(x = "Year", y = "Percentage of Jails Filled", title = "Percentage of Jails Filled in Maine From 1970 to 2018") +
  theme(legend.position = "top") +
  scale_color_viridis_d()
...
p + 
  geom_point() +
  transition_reveal(year)
anim_save("me_percentage_filled.gif")
...
colnames(prison_expenditures2) <- c("state", "Year_2015", "state_res_2015", "incarc_rate_2015", "cost_per_res_2015", "prison_pop_2010", "prison_pop_2015", "change_in_prison_pop", "expenditures_2010", "expenditures_2015", "change_in_expenditures", "salaries_overtime_share_2015", "personnel_services_share_2015", "boarding_payments_share_2015", "healthcare_share_2015", "other_costs_share_2015")
prison_expenditures2 <- prison_expenditures2[-c(46:102), ]
prison_expenditures2 <- prison_expenditures2 %>% 
  select((state:other_costs_share_2015))

prison_expenditures2 <- prison_expenditures2 %>%
  mutate(Year_2010 = as.numeric(gsub(",", "", expenditures_2010))/as.numeric(gsub(",", "", prison_pop_2010)))

prison_expenditures2 <- prison_expenditures2 %>%
  mutate(Year_2015 = as.numeric(gsub(",", "", Year_2015)))

prison_expenditures2 <- pivot_longer(prison_expenditures2, cols = c("Year_2010", "Year_2015"), names_to = "Year")
...
m <- prison_expenditures2 %>%
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(
    x = "Year", y = "Average Cost Per Inmate", fill = "Year", title = "Average Cost Per Inmate in New England in 2010 and 2015"
    ) + 
  scale_fill_viridis_d() +
  facet_wrap(~ state, ncol = 4)

png(file="prison_expenditures.png", width=500, height=500)
m
dev.off()
...
```{r}
percentage_new_england <- incarc_trends_2 %>%
  filter(state %in% c("ME", "VT", "NH", "RI", "CT", "MA")) %>%
  select(state, year, jail_rated_capacity, total_jail_pop) %>%
  group_by(state, year) %>%
  summarise(percent_filled = (sum(total_jail_pop, na.rm = TRUE))/((sum(jail_rated_capacity, na.rm = TRUE))))
...
p <- ggplot(
  ...
  geom_line() +
  labs(x = "Year", y = "Percent Filled", title = "Percentage of Jail Filled in Maine") +
  theme(legend.position = "top") +
  scale_color_viridis_d()
